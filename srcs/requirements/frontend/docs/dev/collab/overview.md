...
- **Frontend WebSocket client**: A custom `WebSocketProvider` (`src/collaboration/WebSocketProvider.ts`) connects to `/api/notes/{id}/sync` using our binary protocol. It handles sync, awareness, reconnection with exponential backoff, and offline edit reconciliation. See [WebSocket Provider Design](./websocket-provider.md).
- **Rich text editor**: Tiptap is integrated via `NoteEditor.vue` with `@tiptap/extension-collaboration` (Yjs document binding) and `@tiptap/extension-collaboration-caret` (remote cursor rendering). The `useCollaboration` composable (`src/composables/useCollaboration.ts`) manages Y.Doc, Awareness, and provider lifecycle. See [Tiptap + Yjs Integration](./tiptap-yjs-integration.md).
- **Nginx WebSocket proxy**: The `/api/notes` location block includes `Upgrade` / `Connection` headers, so WebSocket connections to `/api/notes/{id}/sync` pass through correctly.
- **Live cursors**: Remote user cursors and selections are rendered in real time via the Yjs Awareness protocol. See [Live Cursors](./live-cursors.md).
...
- **Content type mismatch**: The backend creates the `"content"` field as `Y.Text` (`get_or_insert_text`), but Tiptap's Collaboration extension uses `Y.XmlFragment` internally when configured with `document: ydoc`. This works because `Collaboration.configure({ document: ydoc })` creates its own `XmlFragment` field (named `"default"`), bypassing the `"content"` Text field. The `"content"` Text field created at REST note creation is effectively unused. This should be cleaned up — either remove the `"content"` Text initialization from the backend, or align the field names.
