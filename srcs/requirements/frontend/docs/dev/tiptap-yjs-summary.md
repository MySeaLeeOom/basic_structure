...
Quick-reference guide. For detailed design docs, see the [collab/](collab/) directory: [overview](collab/overview.md), [tiptap-yjs-integration](collab/tiptap-yjs-integration.md), and [live-cursors](collab/live-cursors.md).
...
| File | Role |
|---|---|
| `app/components/notes/NoteEditor.vue` | UI — Tiptap editor + title input, consumes the composable |
| `app/composables/useCollaboration.ts` | Glue + Transport — accepts a noteId getter; creates Y.Doc and WebsocketProvider (from `y-websocket`); watches for note changes and seamlessly swaps connections after first sync |
...
Presentation + sidebar sync. Destructures the composable's return value and binds it to the template. Always editable (no view/edit toggle). Watches `titleText` and calls `noteStore.updateNoteTitle()` to keep the sidebar Listbox in sync and debounce a PUT to Postgres for cross-tab persistence.
...
1. **Anonymous users** — all carets show "Anonymous" with same color. Needs auth integration.
2. **No offline persistence** — local edits survive reconnect (in-memory) but are lost if tab closes while disconnected. Fix: add `y-indexeddb`.
